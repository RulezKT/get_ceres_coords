...
CERES_FILE_RECORDS: tuple[FileRecords] = (
    ...
    },
    {
        ...
        # "rsize": 20,
        # init =  -2768989003.525376
        # intlen =  -2638708105.026347 ???
        # n =  415.0
    },
    {
        # 7
        "rec_start_addr": 276_286,
        "seg_start_time": 631_108_800,
        "seg_last_time": 1_274_239_976,
        "int_len": 1_274_239_976,
        "rec_last_addr": 322_292,
        # "rsize": 20,
        # init =  1141570218.9406343
        # n =  500.0
    },
    {
        # 8
    ...
    },
    {
        ...
        # "rsize": 20,
        # init =  2963601497.138017
        # intlen =  3092529717.6804357 ?????
        # n =  474.0
    },
)
...
def get_type21_coords(dateInSeconds, ceres_file: bytes):
    ...
    for i_summ in range(1, total_summaries_number + 1):
        if (
            CERES_FILE_RECORDS[i_summ]["seg_start_time"] < dateInSeconds
            and CERES_FILE_RECORDS[i_summ]["seg_last_time"] > dateInSeconds
        ):
            ...
            arrayInfoOffset = (CERES_FILE_RECORDS[i_summ]["rec_last_addr"] - 4) * 8
            ...
            arrayInfo = array.array(
                "d", ceres_file[arrayInfoOffset : arrayInfoOffset + 32]
            )
            # init = arrayInfo[0]
            # print("init = ", init)
            # intlen = arrayInfo[1]
            ...
            # start_adress = CERES_FILE_RECORDS[i_summ]["rec_start_addr"]
            # last_adress = CERES_FILE_RECORDS[i_summ]["rec_last_addr"]
            start_adress = CERES_FILE_RECORDS[i_summ]["rec_start_addr"]
            last_adress = CERES_FILE_RECORDS[i_summ]["rec_last_addr"]
            ...
            if n_of_rec <= BUFSIZ:
                ...
                data = array.array(
                    "d", ceres_file[(OFFE + 1) * 8 : (OFFE + n_of_rec) * 8 + 8]
                )[0]
                RECNO = LSTLTD(dateInSeconds, data)
                OFFR = start_adress - 1 + (RECNO + 1) * DFLSIZ
                final_record = array.array(
                    "d", ceres_file[(OFFR) * 8 : (OFFR + DFLSIZ) * 8]
                )

            else:
                for dir in range(0, n_of_dir):
                    data = array.array(
                        "d", ceres_file[(OFFD + dir) * 8 : (OFFD + dir) * 8 + 8]
                    )[0]
                    if data > dateInSeconds:
                        # print("we have found , using dirs")
                        # print("data = ", data)
                        # print("dateInSeconds = ", dateInSeconds)
                        OFFD = OFFE + (dir) * BUFSIZ
                        data = array.array(
                            "d", ceres_file[(OFFD) * 8 : (OFFD + BUFSIZ) * 8]
                        ).tolist()
                        # print("data2 = ", data)
                        # v2 index_of_rec = LSTLTD(dateInSeconds, data) +1
                        index_of_rec = LSTLTD(dateInSeconds, data)
                        RECNO = dir * BUFSIZ + index_of_rec
                        # print("index_of_rec= ", index_of_rec)
                        # print(data[index_of_rec])
            ...
            # if ndir >= 1:
            #     epoch_dir = array.array(
            #         "d",
            #         ceres_file[(last_adress - ndir - 1) * 8 : (last_adress - 2) * 8],
            #     )
            #     found = False
            #     for i in range(1, ndir + 1):
            #         if epoch_dir[i - 1] > dateInSeconds:
            #             found = True
            #             break
            #     if found:
            #         search_last_index = i * 100
            #         search_start_index = (i - 1) * 100 + 1
            #     else:
            #         search_last_index = n
            #         search_start_index = ndir * 100 + 1

            # else:
            #     search_last_index = n
            #     search_start_index = 1

            # epoch_table = array.array(
            #     "d",
            #     ceres_file[
            #         (start_adress + (n * DFLSIZ)) * 8 : (
            #             start_adress + (n * DFLSIZ) + n - 1
            #         )
            #         * 8
            #     ],
            # )
            ...
            # for i in range(search_start_index, search_last_index + 1):
            #     if epoch_table[i - 1] > dateInSeconds:
            #         found = True
            #         break
            # if not found:
            #     i = search_last_index
            ...
            # # ?????? start_adress - 1
            # py_final_record = array.array(
            #     "d",
            #     ceres_file[
            #         (start_adress + (record_index - 1) * DFLSIZ) * 8 : (
            #             (start_adress + record_index * DFLSIZ - 1) * 8
            #         ) :
            #     ],
            # )
            # -----------------------------------------
            ...
            # epoch_table = self.daf.map_array(
            #     self.start_i + (n * rsize),
            #     self.start_i + (n * rsize) + n - 1,
            # )

            # Initial address for the epoch table (after all the segment records)
            # etid = int(
            #     (CERES_FILE_RECORDS[i]["rec_start_addr"] - 1) * 8 + 8 * rsize * n
            # )

            # serch target epoch in epoch directory to narrow serching aria

            # if ndir >= 1:
            #     start_pos = int(CERES_FILE_RECORDS[i_summ]["rec_last_addr"] - ndir * 8)
            #     end_pos = int(CERES_FILE_RECORDS[i_summ]["rec_last_addr"] - 8)
            #     epoch_dir = array.array(
            #         "d",
            #         chiron_file[start_pos:end_pos],
            #     )
            #     found = False
            #     for i in range(1, ndir + 1):
            #         if epoch_dir[i - 1] > dateInSeconds:
            #             print("ndir found ! :", i - 1)
            #             print(epoch_dir[i - 1])
            #             found = True
            #             break
            #     if found:
            ...
            # serch_start_index = 1
            # etid_start = 8 * int(
            #     (CERES_FILE_RECORDS[i_summ]["rec_start_addr"]) + rsize * n
            # )
            # etid_end = 8 * int(
            #     (CERES_FILE_RECORDS[i_summ]["rec_start_addr"]) + rsize * n + n - 1
            # )
            # epoch_table = array.array("d", chiron_file[etid_start:etid_end])

            # # serch target epoch in epoch_table
            # found = False
            # # record_index = 0
            # for k in range(serch_start_index, serch_last_index + 1):
            #     if epoch_table[k - 1] > dateInSeconds:
            #         print("epoch found!")
            #         print(epoch_table[k - 1])
            #         print(epoch_table[k - 2])
            #         found = True
            #         # record_index = k - 2
            #         break
            # if not found:
            ...
            # upper_boundary = epoch_table[i - 1]  print(epoch_table[i - 1])
            # if i != 1:
            #     lower_boundary = epoch_table[i - 2]
            # else:
            #     lower_boundary = CERES_FILE_RECORDS[i]["seg_start_time"]

            # print("i_summ = ", i_summ)
            # print("rsize = ", rsize)
            # print("record_index = ", record_index)
            # print(epoch_table[record_index - 2])
            # print(epoch_table[record_index - 1])
            # mba_start = 8 * int(
            #     CERES_FILE_RECORDS[i_summ]["rec_start_addr"]
            #     + ((record_index) * rsize)
            #     + 1
            # )
            # mba_end = 8 * int(
            #     CERES_FILE_RECORDS[i_summ]["rec_start_addr"]
            #     + ((record_index + 1) * rsize + 3)
            # )

            # mba_start = 8 * int(CERES_FILE_RECORDS[i_summ]["rec_start_addr"])
            # mba_end = 8 * int(CERES_FILE_RECORDS[i_summ]["rec_start_addr"] + rsize + 1)

            # mba_start = 8 * int(
            #     CERES_FILE_RECORDS[i_summ]["rec_start_addr"]
            #     + (record_index - 1) * rsize
            #     - 2
            # )
            # mba_end = 8 * int(
            #     CERES_FILE_RECORDS[i_summ]["rec_start_addr"]
            ...
            # mda_record = array.array(
            #     "d",
            #     chiron_file[mba_start:mba_end],
            # )
